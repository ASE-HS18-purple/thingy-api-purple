...
import {AirQualityEvent, HumidityEvent, PressureEvent, TemperatureEvent} from './ThingyNotifyEvents';
...
describe('Eventbus', () => {
    ...
    it('should should subscribe and fire to events', function() {
        ...
        eventbus.subscribeToTemperature(temperatureSpy, 1);
        eventbus.subscribeToHumidity(humiditySpy, 2);
        eventbus.subscribeToAirQuality(airQualitySpy, 3);
        eventbus.subscribeToPressure(pressureSpy, 4);
        let temperatureEvent = new TemperatureEvent(0, 1, 5);
        let humidityEvent = new HumidityEvent(0, 2, 6);
        let airQualityEvent = new AirQualityEvent(0, 3, 7);
        let pressureEvent = new PressureEvent(0, 4, 8);
        eventbus.fireTemperatureEvent(temperatureEvent, 1);
        eventbus.fireHumidityEvent(humidityEvent, 2);
        eventbus.fireAirQualityEvent(airQualityEvent, 3);
        eventbus.firePressureEvent(pressureEvent, 4);
        setTimeout(() => {
